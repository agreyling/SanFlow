...
uses
  ...
  Menus, StdCtrls, ExtCtrls, Grids, ComCtrls, Math, Buttons, ToolWin, ActnList,
  ActnMan, ActnCtrls, System.Actions, System.UITypes,
  PlatformDefaultStyleActnCtrls, Vcl.XPMan,
...
type
  ...
  private
    ...
    procedure ShowDlgZoneConstants;
    procedure ShowDlgVariables;
    procedure ShowDlgReport;
    procedure ShowDlgSensitvity;
    procedure ShowDlgBackgroundLosses;
    procedure ShowDlgNormalNightUse;
  end;
...
implementation
uses
  fmAbout, fmZoneConstants, fmZoneVariables, fmBackgroundLosses,
  fmNormalNightUse, fmGraph, fmReport, fmStats;
...
{   Data Load, Save, New
{------------------------------------------------------------------------------}
procedure TfrmMain.LoadData(AFilename : string);
var
  r : integer;
begin
...
begin
  ...
  while NOT(EOF(f)) do begin
    ...
    while (s <> '[Zone Variables]') and NOT(EOF(f)) do begin
      ...
      CurrentMetered.Description := uGlobals.GetPortion(s);
      readln(f,s); CurrentMetered.Use := StrToReal(uGlobals.GetPortion(s));
      readln(f,s);
    end;
...
procedure TfrmMain.CreateNewLine;
var
...
{------------------------------------------------------------------------------}
procedure TfrmMain.SaveDataFile(AFilename : string);
var
  f : textfile;
begin
  CurrentZone := ZoneHeadPtr;
...
{   Grid
{------------------------------------------------------------------------------}
procedure TfrmMain.GridInitialise;
begin
...
begin
  frmMain.Caption := 'South African Night Flow Analysis - '+ZoneName;
  GridUpdate;
end;

procedure TfrmMain.CalculateValues;
var
...
{ Grid }
procedure TfrmMain.MainGridMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  col, row : integer;
begin
...
procedure TfrmMain.MainGridKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  r : integer;
begin
  //delete
...
procedure TfrmMain.MainGridMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  col, row, r : integer;
begin
  if Button = mbRight then begin
    ...
    //create new row
      ...
      for r := (MainGrid.FixedRows) to (row - 1) do
        CurrentZone := CurrentZone.Next;
      case col of
        4 : frmBackgroundLosses.Show;
        5 : frmNormalNightUse.Show;
        else
          frmZoneVariables.Show;
      end;//case
    end;//if
  end;//if
end;
...
begin
  ...
  { Edit }
  if sender = actZoneConstants    then ShowDlgZoneConstants;
  if sender = actZoneVariables    then ShowDlgVariables;
  if sender = actNormalNightUse   then ShowDlgNormalNightUse;
  if sender = actBackgroundLosses then ShowDlgBackgroundLosses;
  if sender = actNewLine          then CreateNewLine;
  { Tools }
  if sender = actReports          then ShowDlgReport;
  if sender = actGraphs           then frmGraph.Show;
  if sender = actSensitivity      then ShowDlgSensitvity;
  { Help }
  if sender = actHelpAbout        then frmAbout.Show;
...
{   Show dialog forms
{------------------------------------------------------------------------------}
procedure TfrmMain.ShowDlgZoneConstants;
begin
  frmZoneConstants.ShowModal;
  UpdateScreen;
end;

procedure TfrmMain.ShowDlgBackgroundLosses;
var
  r : integer;
begin
  CurrentZone := ZoneHeadPtr;
  //for r := (MainGrid.FixedRows) to (MainGrid.Row-1) do
  //  CurrentZone := CurrentZone.Next;
  frmBackgroundLosses.ShowModal;
  UpdateScreen;
end;

procedure TfrmMain.ShowDlgNormalNightUse;
var
  r : integer;
begin
  CurrentZone := ZoneHeadPtr;
  for r := (MainGrid.FixedRows) to (MainGrid.Row-1) do
    CurrentZone := CurrentZone.Next;
  frmNormalNightUse.ShowModal;
  UpdateScreen;
end;

procedure TfrmMain.ShowDlgSensitvity;
var
  r : integer;
begin
  CurrentZone := ZoneHeadPtr;
...
procedure TfrmMain.ShowDlgReport;
begin
  frmReport.Start := MainGrid.Selection.Top - MainGrid.FixedRows;
  frmReport.Finish := MainGrid.Selection.Bottom - MainGrid.FixedRows;
  frmReport.ShowModal;
  UpdateScreen;
end;

procedure TfrmMain.ShowDlgVariables;
var
  r : integer;
begin
  CurrentZone := ZoneHeadPtr;
