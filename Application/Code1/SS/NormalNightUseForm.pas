unit NormalNightUseForm;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, Grids, Globals, ExtCtrls;
...
uses UnMeteredEditorForm, MeteredEditorForm;
...
begin
  ...
  begin
    ...
    begin
      ...
      while ZoneUnMetered.Next <> nil do
      begin
      ...
      UnMeteredEditorFm.Show;
    end//if
    else if ThisRow > (UnMeteredGrid.FixedRows - 1) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      ZoneMetered.Use := 0.0;
      MeteredEditorFm.Show;
    end//if
    else if ThisRow > (MeteredGrid.FixedRows - 1) then
    begin
...
procedure TfrmNormalNightUse.FormActivate(Sender: TObject);
var
...
begin
  LastPtr := nil;
  if (Key = 46) and (UnmeteredGrid.RowCount > (UnmeteredGrid.FixedRows+1)) then //delete
  begin
    ...
    if (LastPtr = nil) then//at header node
    begin
...
begin
  LastPtr := nil;
  if (Key = 46) and (MeteredGrid.RowCount > (MeteredGrid.FixedRows+1)) then //delete
  begin
